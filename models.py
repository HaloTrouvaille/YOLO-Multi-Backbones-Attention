...
def create_modules(module_defs, img_size, arc):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'quantize_convolutional':
            ...
            if mdef['activation'] == 'leaky':  # TODO: activation study https://github.com/ultralytics/yolov3/issues/441
                modules.add_module('activation', nn.LeakyReLU(0.1, inplace=True))
            if mdef['activation'] == 'HardSwish':
                modules.add_module('activation',HS())
            ...
            filters = int(mdef['out_channels'])
            modules.add_module('Stage2', ShuffleUnit(24, 176, 2,nn.LeakyReLU(0.1,inplace=True)))
            modules.add_module('Stage2_1', ShuffleUnit(176, 176, 1,nn.LeakyReLU(0.1,inplace=True)))
            modules.add_module('Stage2_2', ShuffleUnit(176, 176, 1,nn.LeakyReLU(0.1,inplace=True)))
            modules.add_module('Stage2_3', ShuffleUnit(176, 176, 1,nn.LeakyReLU(0.1,inplace=True)))
        elif mdef['type'] == 'stage3':
            filters = int(mdef['out_channels'])
            modules.add_module('Stage3', ShuffleUnit(176, 352, 2,HS()))
            modules.add_module('Stage3_1', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_2', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_3', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_4', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_5', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_6', ShuffleUnit(352, 352, 1,HS()))
            modules.add_module('Stage3_7', ShuffleUnit(352, 352, 1,HS()))
        elif mdef['type'] == 'stage4':
            filters = int(mdef['out_channels'])
            modules.add_module('Stage4', ShuffleUnit(352, 704, 2,HS()))
            modules.add_module('Stage4_1', ShuffleUnit(704, 704, 1,HS()))
            modules.add_module('Stage4_2', ShuffleUnit(704, 704, 1,HS()))
            modules.add_module('Stage4_3', ShuffleUnit(704, 704, 1,HS()))
...
class Darknet(nn.Module):
    ...
    def forward(self, x, var=None):
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            mtype = mdef['type']
            if mtype in ['convolutional', 'quantize_convolutional','upsample', 'maxpool', 'stage2', 'stage3', 'stage4', 'se', 'ca', 'sa', 'eca']:
                x = module(x)
            elif mtype == 'route':
                layers = [int(x) for x in mdef['layers'].split(',')]
                if len(layers) == 1:
